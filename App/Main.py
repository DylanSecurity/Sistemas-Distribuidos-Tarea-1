...
async def q1_proximos_partidos(equipo: str):
    ...
    try:
        ...
        if cached_data:
            ...
            await registrar_hit()
            await registrar_latencia(latencia)
            return {
                "origen": "CACHE (Hit)",
    ...
    try:
        ...
        await r.setex(cache_key, TTL_SEGUNDOS, json.dumps(datos))
        latencia = (time.perf_counter() - start_time) * 1000
        await registrar_miss()
        await registrar_latencia(latencia)
        return {
            "origen": "SCRAPER (Miss)",
            "tiempo_ms": round(latencia, 2),
...
async def q3_historial_enfrentamientos(equipo1: str, equipo2: str):
    ...
    try:
        ...
        if not datos:
            raise HTTPException(
                status_code=404, detail="No se encontraron enfrentamientos"
            )
...
async def q4_partidos_periodo(fecha_inicio: str, fecha_fin: str):
    ...
    try:
        ...
        if not datos:
            raise HTTPException(
                status_code=404, detail="No se encontraron partidos en el periodo"
            )
...
async def q5_tabla_posiciones():
    ...
    try:
        ...
        if not datos:
            raise HTTPException(
                status_code=404, detail="No se pudo obtener la tabla de posiciones"
            )
